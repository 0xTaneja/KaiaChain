...
async function getLatestBlock(connection) {
  try {
    const blockNumberHex = await callKaiaRPC(
      "kaia_blockNumber",
      [],
      connection
    );
...
async function getLastTransactions(address, count, network) {
  const connectiontouse =
    network === "testnet" ? testnetprovider : mainnetprovider;
  try {
    const transactions = [];
    ...
    const allTransactions = await getTransactionsForRange(
      blockRange,
      address,
      connectiontouse
    );
...
async function getTransactionInfo(txHash, network) {
  ...
  try {
    const transactions = [];
    const transaction = await callKaiaRPC(
...
const signTransactionWithKaiaWallet = async (tx) => {
  if (typeof window !== "undefined" && typeof window.kaia !== "undefined") {
    try {
      ...
      throw new Error(
        "Transaction signing failed. Ensure Kaia Wallet is connected."
      );
    }
  } else {
...
const sendTransactionKaiaViaWallet = async (
  ...
  try {
    ...
    if (!senderAddress || !receiverAddress || !amount) {
      throw new Error(
        "Invalid parameters. Ensure sender, receiver, and amount are provided."
      );
    }
...
async function generateHumanReadableResponse(result, network, action) {
  let resultString = "";
...
Price: 1 ${result.fromToken} = ${(
        (quote.outAmount / (result.amount * 1e9)) *
        1e6
      ).toFixed(6)} ${result.toToken}
